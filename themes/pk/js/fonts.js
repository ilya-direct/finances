define("fonts/utils/fontMetadata", [], function () {
    ...
    function k(a) {
        var b = a.split(" "), c = b[3] ? b[3].split("/") : [];
        return{style: b[0], variant: b[1], weight: b[2], size: c[0], lineHeight: c[1], family: b[4].replace(/\+/g, " "), color: b[5], bold: "bold" === b[2] || parseInt(b[2], 10) >= 700, italic: "italic" === b[0]}
    }
    ...
    function m(c) {
        return a.compact(a.map(c, function (a) {
            return b[a]
        }))
    }

    function n(a) {
        var c = a.replace(/\+/g, " ").toLowerCase(), d = b[c], e = d.fallbacks;
        return e && (e += ","), e += d.genericFamily
    }

    function o(b) {
        var c = b.split(",");
        return c = a.map(c, function (a) {
            return a.replace(/.*[^\w\d\-].*/, '"$&"')
        }), c.join(",")
    }

    function p(a) {
        var e, c = b[a.toLowerCase()], d = c.fontFamily;
        return c ? (e = d, "" !== c.fallbacks && (e += "," + c.fallbacks), e += "," + c.genericFamily) : e = a, o(e)
    }

    function q(b, d) {
        if (0 === b.length)return void 0;
        var e = a.map(b, function (a) {
            return a.cdnName + ":n,b,i,bi|"
        });
        return d && (e.push("&subset="), e.push(d.join(","))), c + e.join("")
    }
    ...
    function u(b) {
        return a.reduce(b, function (a, b) {
            e.lastIndex = 0;
            for (var c; c = e.exec(b);)a.push(c[2].replace(/['"]/g, ""));
            return a
        }, [])
    }

    function v(c, d) {
        var e = c.getPageData(d).data.document_data, f = a(e).filter(function (b) {
            return a.has(g, b.type)
        }).map(function (b) {
            var d = g[b.type](c, b);
            return a.union(u(d))
        }).flatten().value(), h = a(e).filter(function (b) {
            return a.has(i, b.type)
        }).map(function (a) {
            return i[a.type](c, a) || []
        }).flatten().value(), j = a.union(f, h, t(c));
        return a.filter(j, function (c) {
            return a.has(b, c)
        })
    }

    function w(b, c) {
        var d = b.getGeneralTheme();
    ...
    function x(a, b) {
        var c = a.font, d = parseInt(b.substring(b.indexOf("_") + 1), 10), e = c[d], f = "";
        return e && (f = k(e).family.toLowerCase()), f
    }

    function y(a) {
        var e, c = b[a.toLowerCase()], d = c && c.fontFamily;
        return c ? (e = d, "" !== c.fallbacks && (e += "," + c.fallbacks), e += "," + c.genericFamily) : e = a, o(e)
    }

    function z(b, c) {
        var d = [], e = "/";
        b = a.contains(b, "localhost") ? b : b.replace("http://", "//"), "/" === b.charAt(b.length - 1) && (e = "");
       // var f = b + e + "static/css/user-site-fonts/";
        var f = window.cssfolder+ '/';
        if (c)for (var g = 0; g < c.length; g++)d.push(f + c[g] + ".css");
        return d
    }

    function A(a) {
        return z(a.santaBase, a.getDataByQuery("SITE_STRUCTURE").characterSets)
    }

    function B(b, c) {
        for (var e = [], f = D(b), g = 0; g < d.length; g++) {
            var h = d[g];
            a.contains(c, h) && e.push({lang: h, fonts: f[h]})
        }
        return e
    }

    function C(a) {
        return B(a.rendererModel.siteInfo.documentType, a.getDataByQuery("SITE_STRUCTURE").characterSets)
    }

    function D(c) {
        var d = ["all", "legacy"];
        "WixSite" === c && d.push("studio");
        var e = a.reduce(b, function (b, c, e) {
            var f = c.characterSets;
            return a.contains(d, c.permissions) && (c.cssFontFamily = p(e), a.forEach(f, function (a) {
                b[a] || (b[a] = []), b[a].push(c)
            }, this)), b
        }, {});
        return a.forOwn(e, function (b, c) {
            e[c] = a.sortBy(b, function (a) {
    ...
    function E(b, d, e) {
        var f = a.isArray(b) ? b : a.keys(b), g = H(f, d, e);
        return g ? c + g : ""
    }

    function F(a, b) {
        return E(a, b.rendererModel.siteInfo.documentType, b.getDataByQuery("SITE_STRUCTURE").characterSets)
    }

    function G(a) {
        var b = ["all", "legacy"];
        return"WixSite" === a && b.push("studio"), b
    }

    function H(c, d, e) {
        var f = "", g = G(d);
        return a.forEach(c, function (c) {
            var d = b[c];
            d && d.cdnName && a.contains(g, d.permissions) && (f += d.cdnName, f += ":n,b,i,bi|")
        }), "" === f ? null : (e && (f += "&subset=" + e.join(",")), f)
    }
    ...
    function K(a, b) {
        var c = a.color && a.color.match(/{([^}]+)}/);
        return b && c && b[c[1]] ? a.cssColor = b[c[1]] : a.cssColor = a.color, a
    }

    function L(a) {
        var b = k(a);
        return b.fontWithFallbacks = y(b.family), b
    }

    var c = "//yii2"+window.cssfolder+"/googlefont.css?family=", d = ["latin-ext", "cyrillic", "japanese", "korean", "arabic", "hebrew", "latin"], e = /(<[^>]+["']font-family:\s*)([^,;]+)([,;])/g, f = /<[^>]+class="[^"]*(font_[0-9]+)[^"]*"/g, g = {StyledText: function (a, b) {
        return[b.text]
    }, RichText: function (a, b) {
        return[b.text]
    }}, i = {};
    return{parseFontStr: k, parseStyleFont: J, getMetadata: m, getGoogleFontsUrl: q, getFontFamily: l, getFontFallback: n, getPageFontsMetaData: r, getFontFamilyWithFallbacks: y, getWixStoredFontsCssUrls: A, getWixStoredFontsCssUrlsWithParams: z, getCurrentSelectablefonts: C, getCurrentSelectablefontsWithParams: B, getFontsUrl: F, getFontsUrlWithParams: E, getFontFamilyByStyleId: x, getFontClassName: I, registerCustomTextDataGetter: h, registerCustomFontFamiliesGetter: j, getPageUsedFontsList: s, collectFontsFromTextDataArray: u}
}), define("fonts/utils/fontCss", ["lodash", "fonts/utils/fontUtils"], function (a, b) {
    "use strict";
    function c(a) {
        var c = a + "," + b.getFontFallback(a);
        return c = c.replace(/[^,]*[^\w,\d\-][^,]*/g, function (a) {
            return"'" + a.replace(/\+/g, " ") + "'"
        })
    }

    function d(a, b) {
        var c = a.match(/{color_(\d+)}/);
        if (!c) {
            var d = a.match(/#[A-Z0-9]{3,6}/);
            return d ? "color:" + d[0] + ";" : ""
        }
        var e = c[1], f = b.color[e];
        return 0 === f.indexOf("#") ? "color:" + f + ";" : "color:rgba(" + f + ");"
    }

    function e(d) {
        var e = d;
        a.contains(e, "#") && (e = e.slice(0, e.indexOf("#"))), e = e.replace(/\{color_\d+\}/, "");
        var f = b.getFontFamily(e), g = c(f), h = e.replace(f, g);
        return h + ";"
    }

    function f(a, b) {
        var c = a.split("font_");
        return 2 === c.length ? b.font[c[1]] : a
    }

    function g(a, b) {
        var c = f(a), g = e(c), h = d(c, b);
        return g + h
    }
    ...
    return{fontToCSSWithColor: g, fontToCSSWithoutColor: h}
}), define("fonts", ["fonts/utils/fontUtils", "fonts/utils/fontCss", "fonts/utils/fontMetadata"], function (a, b, c) {
    "use strict";
    return{fontUtils: a, fontCss: b, fontMetadata: c}
});